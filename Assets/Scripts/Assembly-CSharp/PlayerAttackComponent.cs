...
public class PlayerAttackComponent : AttackComponent
{
	...
	public void save(string levelToSaveAt)
	{
		...
		PlayerPrefs.SetInt("knockDownProbabiliy", knockDownProbabiliy);
		PlayerPrefs.SetFloat("strength", strength);
		PlayerPrefs.SetFloat("recoveryRate", recoveryRate);
		PlayerPrefs.SetInt("attack1Level", attack1Level);
		PlayerPrefs.SetInt("attack2Level", attack2Level);
		PlayerPrefs.SetInt("attack3Level", attack3Level);
		PlayerPrefs.SetInt("weaponAttack1Level", weaponAttack1Level);
		PlayerPrefs.SetInt("weaponAttack2Level", weaponAttack2Level);
		PlayerPrefs.SetInt("weaponAttack3Level", weaponAttack3Level);
		PlayerPrefs.SetInt("counterAttackLevel", counterAttackLevel);
		PlayerPrefs.SetFloat("locX", base.transform.position.x);
		PlayerPrefs.SetFloat("locY", base.transform.position.y);
		PlayerPrefs.SetFloat("locZ", base.transform.position.z);
		PlayerPrefs.SetFloat("rotX", base.transform.rotation.x);
		PlayerPrefs.SetFloat("rotY", base.transform.rotation.y);
		PlayerPrefs.SetFloat("rotZ", base.transform.rotation.z);
		PlayerPrefs.SetFloat("rotW", base.transform.rotation.w);
		...
		PlayerPrefs.SetString("savedItemList", text);
		string text2 = string.Empty;
		for (int j = 0; j < equippedItems.Count; j++)
		{
			BadNerdItem item = equippedItems[j];
			text2 = text2 + itemList.IndexOf(item) + ",";
		}
		PlayerPrefs.SetString("savedEquippedList", text2);
		string text3 = string.Empty;
		int num = 0;
		foreach (string item2 in questsCompleted)
		{
			text3 += item2;
			num++;
			if (num < questsCompleted.Count)
			{
				text3 += ",";
			}
		}
		PlayerPrefs.SetString("levelsCompleted", text3);
		Debug.Log("LEVELS_COMPLETED: " + text3);
	}
	...
	public void load()
	{
		...
		{
			BadNerdItem badNerdItem = itemList[0];
			badNerdItem.detachItem(false);
			UnityEngine.Object.Destroy(badNerdItem.gameObject);
		}
		string @string = PlayerPrefs.GetString("savedItemList");
		Debug.Log("savedItemListString=" + @string);
		string[] array = @string.Split(',');
		List<BadNerdItem> list = new List<BadNerdItem>();
		for (int i = 0; i < array.Length; i++)
		{
			string text = array[i];
			Debug.Log("itemToInstanciate=" + text);
			if (!text.Equals(string.Empty))
			{
				BadNerdItem badNerdItem2 = VNLUtil.instantiate<BadNerdItem>(text);
				list.Add(badNerdItem2);
				badNerdItem2.loadToItemList(i);
			}
		}
		string string2 = PlayerPrefs.GetString("savedEquippedList");
		string[] array2 = string2.Split(',');
		Debug.Log("itemsToEquip=" + string2);
		List<BadNerdItem> list2 = new List<BadNerdItem>();
		string[] array3 = array2;
		foreach (string text2 in array3)
		{
			Debug.Log("index=" + text2);
			if (!text2.Equals(string.Empty))
			{
				list2.Add(list[int.Parse(text2)]);
			}
		}
		BadNerdItem.ignoreCapacity = true;
		foreach (BadNerdItem item in list2)
		{
			item.bePickedUp(this);
		}
		foreach (BadNerdItem item2 in list)
		{
			if (!list2.Contains(item2))
			{
				Debug.Log("remaining item=" + item2);
				item2.bePickedUp(this);
				item2.putAwayItem();
			}
		}
		BadNerdItem.ignoreCapacity = false;
		string string3 = PlayerPrefs.GetString("levelsCompleted");
		Debug.Log("questsCompletedString=" + string3);
		questsCompleted = string3.Split(',').ToList();
